...
# Constants
AVALANCHEGO_PLUGIN_PATH="/path/to/avalanchego/plugins" # Replace with your actual path

echo "Creating blockchain with custom VM..."
avalanche blockchain create laviniasblockchain \
  --custom \
...
echo "Retrieving VM ID for the blockchain..."
VM_ID=$(avalanche blockchain describe laviniasblockchain | grep "VM ID" | awk '{print $NF}')

if [[ -z "$VM_ID" ]]; then
  echo "Error: Failed to retrieve VM ID."
  exit 1
fi
...
# Renaming the binary
echo "Renaming binary for VM..."
NEW_BINARY_NAME="pjSWfcuH2wL6ZzEwWgk88RhmCFgZAh5yvETtPwx9HBiCyYC41"
mv "$AVALANCHEGO_PLUGIN_PATH/$VM_ID" "$AVALANCHEGO_PLUGIN_PATH/$NEW_BINARY_NAME"
echo "Binary renamed to $NEW_BINARY_NAME."
