...
var (
	tests = map[string]testutils.PrecompileTest{
		"insufficient gas for verifySignature should fail": {
			Caller: common.Address{1},
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				// populate test input here
				testInput := VerifySignatureInput{}
				input, err := PackVerifySignature(testInput)
				require.NoError(t, err)
				return input
			},
			SuppliedGas: VerifySignatureGasCost - 1,
			ReadOnly:    false,
...
// TestBlsVerificationRun tests the Run function of the precompile contract.
func TestBlsVerificationRun(t *testing.T) {
	// Run tests.
