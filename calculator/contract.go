...
var (
	...
	// ICalculatorRawABI contains the raw ABI of ICalculator contract.
	//go:embed contract.abi
	ICalculatorRawABI string

	ICalculatorABI = contract.ParseABI(ICalculatorRawABI)

	ICalculatorPrecompile = createICalculatorPrecompile()
)
...
func UnpackAddInput(input []byte) (AddInput, error) {
	...
	err := ICalculatorABI.UnpackInputIntoInterface(&inputStruct, "add", input, false)
...
func PackAdd(inputStruct AddInput) ([]byte, error) {
	return ICalculatorABI.Pack("add", inputStruct.Value1, inputStruct.Value2)
}

// PackAddOutput attempts to pack given result of type *big.Int
// to conform the ABI outputs.
func PackAddOutput(result *big.Int) ([]byte, error) {
	return ICalculatorABI.PackOutput("add", result)
}
...
func UnpackAddOutput(output []byte) (*big.Int, error) {
	res, err := ICalculatorABI.Unpack("add", output)
	if err != nil {
...
func UnpackNextTwoInput(input []byte) (*big.Int, error) {
	res, err := ICalculatorABI.UnpackInput("nextTwo", input, false)
	if err != nil {
		return new(big.Int), err
	}
	unpacked := *abi.ConvertType(res[0], new(*big.Int)).(**big.Int)
...
func PackNextTwo(value1 *big.Int) ([]byte, error) {
	return ICalculatorABI.Pack("nextTwo", value1)
}
...
func PackNextTwoOutput(outputStruct NextTwoOutput) ([]byte, error) {
	return ICalculatorABI.PackOutput("nextTwo",
...
func UnpackNextTwoOutput(output []byte) (NextTwoOutput, error) {
	...
	err := ICalculatorABI.UnpackIntoInterface(&outputStruct, "nextTwo", output)
...
func UnpackRepeatInput(input []byte) (RepeatInput, error) {
	...
	err := ICalculatorABI.UnpackInputIntoInterface(&inputStruct, "repeat", input, false)
...
func PackRepeat(inputStruct RepeatInput) ([]byte, error) {
	return ICalculatorABI.Pack("repeat", inputStruct.Times, inputStruct.Text)
}

// PackRepeatOutput attempts to pack given result of type string
// to conform the ABI outputs.
func PackRepeatOutput(result string) ([]byte, error) {
	return ICalculatorABI.PackOutput("repeat", result)
}
...
func UnpackRepeatOutput(output []byte) (string, error) {
	res, err := ICalculatorABI.Unpack("repeat", output)
	if err != nil {
		return "", err
	}
...
// createICalculatorPrecompile returns a StatefulPrecompiledContract with getters and setters for the precompile.

func createICalculatorPrecompile() contract.StatefulPrecompiledContract {
	...
	for name, function := range abiFunctionMap {
		method, ok := ICalculatorABI.Methods[name]
		if !ok {
